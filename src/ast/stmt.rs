...
pub trait StmtVisitor<T>: ExprVisitor<T> {
    fn visit_stmt(&mut self, stmt: &Stmt) -> T {
        match stmt {
            ...
            Stmt::If(token, expr, then_branch, else_branch) => self.visit_if(token, expr, then_branch, else_branch.as_ref().map(|b| b.as_ref())),
    ...
    fn visit_if(&mut self, token: &Token, expr: &Expr, then_branch: &Stmt, else_branch: Option<&Stmt>) -> T;
